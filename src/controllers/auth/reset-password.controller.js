...
export function forgotPassword({
  ...
  try {
    ...
    const setRandomToken = async () => {
      ...
      let user = await usersService.getOne({ email: req.body.email });
      if (!user) {
    ...
    const sendForgotPasswordEmail = (user) => {
      ...
      const mailOptions = {
        ...
        html: `Vous recevez cet e-mail car vous (ou quelqu'un d'autre) avez demandé la réinitialisation du mot de passe de votre compte.\n\n
        <h1><strong>    ${token}\n\n</strong></h1>
      ...
      const mailSettings = {
        ...
        loggingError: "ERREUR : impossible d'envoyer un e-mail de mot de passe oublié après la mise à niveau de la sécurité.\n",
        errorType: 'errors',
    ...
    return setRandomToken()
      ...
      .then(() => res.status(httpStatusCodes.OK).json(responseHandler({ message: `An email containing a reset code was sent successfully to ${userEmail}`, msgCode: httpStatusCodes.FORGOT_PASSWORD_EMAIL })))
  ...
  try {
    ...
    const sendResetPasswordEmail = (user) => {
      ...
      const mailOptions = {
        ...
        text: `Bonjour,\n\nCeci est une confirmation que le mot de passe de votre compte ${user.email} vient d'être changé.\n`,
      };

      const mailSettings = {
        successfulType: 'success',
        successfulMsg: 'Succès! Votre mot de passe a été changé.',
        loggingError: "ERREUR : impossible d'envoyer l'e-mail de confirmation de réinitialisation du mot de passe après la mise à niveau de la sécurité.\n",
        errorType: 'warning',
